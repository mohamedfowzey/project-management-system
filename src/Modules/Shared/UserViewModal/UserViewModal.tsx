              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 border-t border-b border-gray-100 dark:border-gray-700 py-3 mb-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400">User Details</p>
                  ...
                  {/* User Account Status */}
                  <div className="space-y-1">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400">Account Status</p>
                  ...
                  {/* title , Status (From the task) */}
                  <div className="space-y-1">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400 mb-2">Task Status</p>

                    {user?.task && user.task.length > 0 && user.task.map((utask, index) => (
                      /* استخدمنا grid وقسمنا السطر لعمودين */
                      <div key={utask?.id || index} className="grid grid-cols-2 items-center gap-1 pb-0.5 border-b border-gray-100 dark:border-gray-800 last:border-0">

                        {/* العمود الأول: عنوان الـ Task (هياخد النص اليمين/الشمال كله بثبات) */}
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">
                          {utask?.title}
                        </span>

                        {/* العمود الثاني: الـ Badge (هتبدأ دايماً من نفس النقطة عمودياً) */}
                        <div>
                          <span
                            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${utask?.status === "ToDo"
                                ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
                                : utask?.status === "InProgress"
                                  ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                                  : "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                              }`}
                          >
                            <span
                              className={`w-2 h-2 rounded-full mr-1.5 ${utask?.status === "ToDo"
                                  ? "bg-amber-500"
                                  : utask?.status === "InProgress"
                                    ? "bg-blue-500"
                                    : "bg-green-500"
                  ...
                  <div className="space-y-1">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400">User Contact</p>
                    <p className="text-sm">
                  ...
                  <div className="space-y-3 col-span-1 md:col-span-2 bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400">Task Timeline</p>
                  ...
                  <div className="col-span-1 md:col-span-2 space-y-2">
                    <p className="text-[14px] font-black uppercase tracking-widest text-gray-400">Task Description</p>
                    <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-300 bg-slate-50 dark:bg-transparent p-3 rounded-lg border border-dashed border-gray-200 dark:border-gray-700">
                      {user?.task[1]?.description || "No description provided for this task."}
                    </p>
                  </div>
                </div>
